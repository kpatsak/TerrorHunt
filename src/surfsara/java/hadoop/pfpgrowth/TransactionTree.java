 ...
 * {@link org.apache.mahout.fpm.pfpgrowth.fpgrowth.FPTree} This reduces plenty
 * of space and speeds up Map/Reduce of {@link PFPGrowth} algorithm by reducing
...
public final class TransactionTree implements Writable,
	...
	public TransactionTree(int[] items, Long support) {
		...
		transactionSet.add(new Pair<IntArrayList, Long>(
				new IntArrayList(items), support));
	}
	...
	public String toString() {
		...
		for (Pair<IntArrayList, Long> transaction : this.transactionSet) {
			finalstring = finalstring.concat(transaction.getFirst().toString());
		}
		return finalstring;
	}
	...
	public TransactionTree getCompressedTree() {
		...
		int node = 0;
		int size = 0;
		List<Pair<IntArrayList, Long>> compressedTransactionSet = Lists
				.newArrayList();
		while (it.hasNext()) {
			Pair<IntArrayList, Long> p = it.next();
			p.getFirst().sort();
	...
	public void readFields(DataInput in) throws IOException {
		...
		if (representedAsList) {
			...
			for (int i = 0; i < numTransactions; i++) {
				...
				Pair<IntArrayList, Long> transaction = new Pair<IntArrayList, Long>(
						new IntArrayList(items), support);
