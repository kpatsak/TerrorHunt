...
import surfsara.java.hadoop.tfidf.SampleFilter;
...
public final class PFPGrowth {
	...
	public static final String ASSOCRULES = "A_RULES";
	...
	public static final Logger log = Logger.getLogger(PFPGrowth.class);
	...
	public static void runPFPGrowth(Parameters params) throws IOException,
		...
		AssociationRulesGen(params, conf);
	}
	...
	public static void AssociationRulesGen(Parameters params, Configuration conf)
			throws IOException, InterruptedException, ClassNotFoundException {
		...
		Job job = new Job(conf, "Association Rules generator running over : "
				+ input);
		job.setJarByClass(PFPGrowth.class);
		...
		job.setMapOutputKeyClass(PatternPair.class);
		job.setMapOutputValueClass(Text.class);
		job.setOutputKeyClass(Text.class);
		job.setOutputValueClass(FloatWritable.class);

		FileInputFormat.addInputPath(job, input);
		Path outPath = new Path(params.get(OUTPUT), ASSOCRULES);
		FileOutputFormat.setOutputPath(job, outPath);
	...
	public static void startParallelCounting(Parameters params,
		...
		log.info("setting input path to '" + input + "'");
